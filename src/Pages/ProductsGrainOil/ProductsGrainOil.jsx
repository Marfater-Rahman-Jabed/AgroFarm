...
const ProductsGrainOil = () => {
    ...
    const { data: grainData = [], refetch } = useQuery({
        queryKey: ['grainDatas'],
        queryFn: async () => {
            setIsLoading(true)
            const res = await fetch(`http://localhost:5000/productsGrainOil`)
            const data = res.json()
            setIsLoading(false)
            return data;
        }
    ...
    const handleDelete = (id) => {
        ...
        fetch(`http://localhost:5000/deleteGrainProducts/${id}`, {
            method: 'DELETE',
            ...
            .then(data => {
                ...
                refetch(`http://localhost:5000/productsGrainOil`)
            })
    }
        ...
        fetch(`https://api.imgbb.com/1/upload?key=${imageKey}`, {
            ...
            .then(image => {
                ...
                if (image.success) {
                    ...
                    fetch(`http://localhost:5000/uploadGrainProduct`, {
                        method: 'POST',
                        headers: {
            ...
            <div className="">
                ...
                <div>
                    <div className='py-24' style={{ backgroundImage: `url(${bgAbout})`, backgroundRepeat: 'no-repeat', backgroundSize: 'cover' }}>
                        <h3 className='text-7xl text-center text-white font-bold'>Our Grain & OilSeed  Items</h3>
                    </div>
                    ...
                    {Admin && <div className='flex justify-end lg:px-20 md:px-10 px-4 py-2'>
                        <button className='btns btn1 border-2 border-solid border-orange-600 px-12 font-bold hover:text-white rounded-lg' onClick={() => document.getElementById('my_modal_Grain').showModal()}>{isUploadLoading ? <Spinner></Spinner> : 'ADD ITEMS'}</button>

                    </div>}
                    {isLoading ? <Loading></Loading> :
                        <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6 lg:px-20 px-6 py-10">
                            {
                                grainData?.filter(data => (data?.name?.toUpperCase().slice(0, searchData.length) === searchData.toUpperCase())).map((product, i) => <Fade key={i} direction="left" duration={2000}>
                ...
                <dialog id="my_modal_Grain" className="modal">
...
export default ProductsGrainOil;